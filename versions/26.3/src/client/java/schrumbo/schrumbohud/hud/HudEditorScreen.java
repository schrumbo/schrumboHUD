...
public class HudEditorScreen extends Screen {
    ...
    public boolean mouseReleased(MouseButtonEvent click) {
        if (click.button() == InputConstants.MOUSE_BUTTON_LEFT && dragging && selected != null) {
            dragging = false;
    ...
    public boolean keyPressed(KeyEvent input) {
        if (selected != null) {
            switch (input.key()) {
                ...
                case InputConstants.KEY_UP -> { nudgeSelected(0, -NUDGE_AMOUNT); return true; }
                case InputConstants.KEY_DOWN -> { nudgeSelected(0, NUDGE_AMOUNT); return true; }
                case InputConstants.KEY_LEFT -> { nudgeSelected(-NUDGE_AMOUNT, 0); return true; }
                case InputConstants.KEY_RIGHT -> { nudgeSelected(NUDGE_AMOUNT, 0); return true; }
            }
        }

...
public class ColorPickerWidget extends Widget {
    ...
    private void renderOpacitySlider(DrawContext context, int sx, int sy, int mouseX, int mouseY) {
        renderCheckerboard(context, sx, sy, SLIDER_WIDTH, SV_PICKER_SIZE, 4);
    ...
    private void renderCheckerboard(DrawContext context, int x, int y, int width, int height, int checkSize) {
        for (int py = 0; py < height; py += checkSize) {
            for (int px = 0; px < width; px += checkSize) {
                boolean isEven = ((px / checkSize) + (py / checkSize)) % 2 == 0;
                int color = isEven ? 0xFFCCCCCC : 0xFF999999;
                int drawWidth = Math.min(checkSize, width - px);
                int drawHeight = Math.min(checkSize, height - py);
                context.fill(x + px, y + py, x + px + drawWidth, y + py + drawHeight, color);
            }
        }
    }

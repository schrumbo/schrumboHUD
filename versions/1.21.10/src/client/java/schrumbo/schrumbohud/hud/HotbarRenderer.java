...
public class HotbarRenderer implements HudElement {
    ...
    public void render(DrawContext context, RenderTickCounter tickCounter) {
        ...
        boolean offhandOnLeft = client.player.getMainArm() == Arm.RIGHT;
    ...
    private void drawPanel(DrawContext context, int ox, int oy, int w, int h, SchrumboHudConfig config) {
    ...
    private void renderSlotBackgrounds(DrawContext context, int rows, int cols, int mainOffX, int mainOffY, int selectedSlot, SchrumboHudConfig config) {
        float t = config.hotbarTransparency;
    ...
    private void renderSingleSlotBg(DrawContext context, int panelX, int panelY, int rawColor, SchrumboHudConfig config) {
    ...
    private void renderOverlays(DrawContext context, PlayerInventory inventory, int rows, int cols, int mainOffX, int mainOffY, SchrumboHudConfig config) {
        int index = 0;
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
    ...
    private void renderOffhandOverlay(DrawContext context, PlayerInventory inventory, int ohOffX, int ohOffY, SchrumboHudConfig config) {
